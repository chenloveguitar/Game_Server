...
//     * @return
//     */
//    public RoomLogic getRoom(int roomId,int type){
...
//    
     ...
     * @return
     */
    public int passIdGetRoom(int uuid){
    	if(roomList != null && roomList.size() > 0){
    		for(Entry<Integer, RoomLogic> entry : roomList.entrySet()){
    			List<Avatar> playerList = entry.getValue().getPlayerList();
    			if(playerList != null && playerList.size() > 0){
    				for(Avatar avatar : playerList){
	    				if(avatar.getUuId() == uuid){
	    					return avatar.getRoomVO().getRoomId();
	    				}
    				}
    			}
     ...
     * @return
     */
    public Map<Integer, RoomLogic> getAllRoom(int roomType){
