...
public class GameSession implements GameObj {
	...
	public void close(){
		if(session != null ) {
			session.close(false);// 2016-8-29断线时不关闭用户session，等
			Avatar avatar = getRole(Avatar.class);
			if(avatar != null){
				if(avatar.isLogOut()){//表明用户已经退出游戏，avatar不是空，则清空
					this.role = null;
					this.session = null;
					avatar = getRole(Avatar.class);
					System.out.println("用户"+avatar.getUuId()+"退出登录后，断开链接");
					return;
				}
				//GameSessionManager.getInstance().removeGameSession(avatar);
				System.out.println("用户"+avatar.getUuId()+"断开链接");
