	...
	public synchronized void process(GameSession gameSession, ClientRequest request) throws Exception {
		if(GlobalUtil.checkIsLogin(gameSession)) {
			...
			if (avatar != null) {
				...
				else if(status.equals("0") && type.equals("1")){
					//随机获取奖品id
					if(account.getRoomcard() >= 88) {
...
//						}
	...
	public void getPrizeInfo(GameSession gameSession,Account account){
		...
		int roomCards = account.getRoomcard() - 88;
		Account accountSqls = new Account();
		accountSqls.setId(account.getId());
		accountSqls.setRoomcard(roomCards);
		account.setRoomcard(roomCards);
		AccountService.getInstance().updateByPrimaryKeySelective(accountSqls);
		gameSession.sendMsg(new RoomCardChangerResponse(1,roomCards));
		//记录中奖信息
...
//			account.setPrizecount(prizecount);
			...
			int roomCard = account.getRoomcard();
			account.setRoomcard(roomCard);
			accountSql.setRoomcard(roomCard);
